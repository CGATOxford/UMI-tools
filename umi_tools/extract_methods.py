...
def extractSeqAndQuals(
    seq, quals, umi_bases, cell_bases, discard_bases, retain_umi=False
):
...
def ExtractBarcodes(
    read, match, extract_umi=False, extract_cell=False, discard=False, retain_umi=False
):
...
class ExtractFilterAndUpdate:
    ...
    def _extract_5prime(self, sequence, read=1):
        if read == 1:
            ...
            return (sequence[: self.pattern_length2], sequence[self.pattern_length2 :])
    ...
    def _getBarcodesRegex(self, read1, read2=None):
        ...
        if self.either_read:  # extract depending on match and match2
            ...
            if match:
                (cell, cell_quals, umi, umi_quals, new_seq, new_quals) = (
                    ExtractBarcodes(
                        read1,
                        match,
                        extract_cell=self.extract_cell,
                        extract_umi=True,
                        discard=True,
                        retain_umi=self.retain_umi,
                    )
                )
            if match2:
                (cell2, cell_quals2, umi2, umi_quals2, new_seq2, new_quals2) = (
                    ExtractBarcodes(
                        read2,
                        match2,
                        extract_cell=self.extract_cell,
            ...
            if match and match2:
                if self.either_read_resolve == "quality":
                    read1_min_quals = min(
                        [
                            x - RANGES[self.quality_encoding][0]
                            for x in map(ord, umi_quals)
                        ]
                    )
    ...
    def _getCellBarcodeRegex(self, read1, read2=None):
        ...
        if not ((self.pattern and not match1) or (self.pattern2 and not match2)):
            cell_barcode1, cell_barcode2 = "", ""

            if self.pattern:
                cell_barcode1 = ExtractBarcodes(
                    read1, match1, extract_cell=True, extract_umi=False
                )[0]
            if self.pattern2:
                cell_barcode2 = ExtractBarcodes(
                    read2, match2, extract_cell=True, extract_umi=False
                )[0]
    ...
    def filterQuality(self, umi_quals):
        if umi_below_threshold(
            umi_quals, self.quality_encoding, self.quality_filter_threshold
        ):
    ...
    def maskQuality(self, umi, umi_quals):
        ...
        masked_umi = mask_umi(
            umi, umi_quals, self.quality_encoding, self.quality_filter_mask
        )
        if masked_umi != umi:
                    ...
                    if corrected_umi:  # if correctable
                        ...
                        self.read_counts[
                            (
                                ...
                                "within threshold to more than one whitelisted UMI"
                            )
                        ] += 1
        ...
        if self.method == "string":
            ...
            if pattern:
                if len(pattern.replace("N", "").replace("X", "").replace("C", "")) > 0:
                    raise ValueError(
                        "barcode pattern (%s) should only contain "
            ...
            if pattern2:
                if len(pattern2.replace("N", "").replace("X", "").replace("C", "")) > 0:
                    raise ValueError(
                        "barcode pattern2 (%s) should only contain "
                        "N/X/C characters" % pattern2
                    )
                self.pattern_length2 = len(pattern2)
                self.umi_bases2 = [
                    x for x in range(len(pattern2)) if pattern2[x] == "N"
                ]
                self.bc_bases2 = [x for x in range(len(pattern2)) if pattern2[x] == "X"]
                self.cell_bases2 = [
                    x for x in range(len(pattern2)) if pattern2[x] == "C"
                ]
    ...
    def __call__(self, read1, read2=None):
        ...
        if self.method == "string":
            if self.pattern and (len(read1.seq) < len(self.pattern)):
                raise ValueError(
                    "Read sequence: %s is shorter than pattern: %s"
                    % (read1.seq, self.pattern)
                )

            if (
                read2 is not None
                and self.pattern2 is not None
                and len(read2.seq) < len(self.pattern2)
            ):
                raise ValueError(
                    "Read2 sequence: %s is shorter than pattern2: %s"
        ...
        if self.either_read:
            new_identifier = addBarcodesToIdentifier(read1, umi, cell, umi_separator)
            read1.identifier = new_identifier

            new_identifier2 = addBarcodesToIdentifier(read2, umi, cell, umi_separator)
