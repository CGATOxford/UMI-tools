...
def recursive_search(node, adj_list):
    ...
    for child in children:
        ...
        recursive_search.component.update(recursive_search(child, adj_list))
...
class ReadDeduplicator:
    ...
    def __call__(self, bundle, threshold):
        ...
        if self.umi_whitelist:
            ...
            umi_counts = [sum(counts[umi] for umi in cluster) for cluster in clusters]
...
class CellClusterer:
            ...
            for umi1 in umis:
                ...
                for umi2 in umis:
                    ...
                    if counts[umi1] >= (counts[umi2] * self.dir_threshold):
                        if (max(len(umi1), len(umi2)) - min(len(umi1), len(umi2))) > 1:
                            continue
                        if comp_regex_err.match(str(umi2)) or comp_regex_del.match(
                            str(umi2)
                        ):
                            adj_list[umi1].append(umi2)
        else:
    ...
    def __init__(
        self, cluster_method="directional", dir_threshold=10, fuzzy_match=True
    ):
        """select the required class methods for the cluster_method
