...
class get_bundles:
    ...
    def __init__(
        ...
        if self.options.get_umi_method == "read_id":
            ...
            self.barcode_getter = partial(
                get_barcode_umis, cell_barcode=self.options.per_cell
            )
        ...
        if self.all_reads:
            ...
            try:
                ...
                if self.options.detection_method in ["NH", "X0"]:
                    ...
                    if self.reads_dict[pos][key][umi]["read"].opt(tag) < read.opt(tag):
                        return
                    elif self.reads_dict[pos][key][umi]["read"].opt(tag) > read.opt(
                        tag
                    ):
                        self.reads_dict[pos][key][umi]["read"] = read
                        self.read_counts[pos][key][umi] = 0
    ...
    def check_output(self):
        ...
        if self.options.per_gene:
            ...
            if self.start > (self.last_pos + 1000) or self.current_chr != self.last_chr:
                do_output = True
                out_keys = sorted(self.reads_dict.keys())
    ...
    def __call__(self, inreads):
        ...
        for read in inreads:
            ...
            if read.is_paired and (read.reference_name != read.next_reference_name):
            ...
            if self.options.ignore_umi:
                ...
                try:
                    ...
                    if self.read_events[error_msg] == 0:
                        ...
                        U.warn(
                            "At least one read is missing UMI and/or "
            ...
            if self.options.per_gene:
                if self.options.per_contig:
                    ...
                    if gene == "":
                        if self.read_events["Read skipped - gene string is empty"] == 0:
                            U.warn(
                                "Assigned gene is empty string. First such "
                                "read:\n%s" % read.to_string()
                            )
                    ...
                    if re.search(self.options.skip_regex, assigned):
                        self.read_events[
                            "Read skipped - assigned tag matches skip_regex"
                        ] += 1
                        continue
                ...
                key = (
                    read.is_reverse,
                    self.options.spliced and is_spliced,
                    (not self.options.ignore_tlen) * self.options.paired * read.tlen,
                    r_length,
                )
...
def get_gene_count_tab(infile, bc_getter=None):
...
class TwoPassPairWriter:
    ...
    def write_mates(self):
        ...
        if self.chrom is not None:
            U.debug("Dumping %i mates for contig %s" % (len(self.read1s), self.chrom))

        for read in self.mate_file.fetch(
            reference=self.chrom, multiple_iterators=False
        ):
    ...
    def close(self):
        ...
        self.write_mates()
        U.info("Searching for mates for %i unmatched alignments" % len(self.read1s))
...
def get_read_position(read, soft_clip_threshold):
    ...
    if read.is_reverse:
        ...
        if "N" in read.cigarstring or (
            read.cigar[0][0] == 4 and read.cigar[0][1] > soft_clip_threshold
        ):
            cigar = read.cigar[::-1]
        ...
        if "N" in read.cigarstring or (
            read.cigar[-1][0] == 4 and read.cigar[-1][1] > soft_clip_threshold
        ):
