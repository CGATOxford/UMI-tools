...
def getKneeEstimateDensity(
    ...
    if plotfile_prefix:
        ...
        fig.savefig(
            "%s_cell_barcode_count_density.png" % plotfile_prefix,
        ...
        if cell_number:
            ...
            for local_mins_count in local_mins_counts:
                fig2.axvline(x=local_mins_count, ls="dashed", color=CB_color_cycle[3])
            lgd = fig2.legend(
                bbox_to_anchor=(1.05, 1),
                loc=2,
                borderaxespad=0.0,
            ...
            for local_mins_count in local_mins_counts:
                if local_mins_count == len(final_barcodes):  # selected local minima
                    fig2.axvline(
                        x=local_mins_count, ls="dashed", color=CB_color_cycle[0]
                    )
                else:
                    fig2.axvline(
                        x=local_mins_count, ls="dashed", color=CB_color_cycle[3]
                    )
        ...
        fig.savefig(
            "%s_cell_barcode_knee.png" % plotfile_prefix,
            bbox_extra_artists=(lgd,),
            bbox_inches="tight",
        )
        ...
        if local_min is not None:
            ...
            colours_rejected = [
                "black" for x in range(0, len(counts) - len(final_barcodes))
            ]
        ...
        fig3.scatter(
            x=range(1, len(counts) + 1), y=counts, c=colours, s=10, linewidths=0
        )
        fig3.loglog()
        ...
        if cell_number:
            ...
            for local_mins_count in local_mins_counts:
                fig3.axvline(x=local_mins_count, ls="dashed", color=CB_color_cycle[3])
            lgd = fig3.legend(
                bbox_to_anchor=(1.05, 1),
                loc=2,
                borderaxespad=0.0,
        ...
        fig.savefig(
            "%s_cell_barcode_counts.png" % plotfile_prefix,
            bbox_extra_artists=(lgd,),
            bbox_inches="tight",
        )
...
def getKneeEstimateDistance(
    cell_barcode_counts, cell_number=False, plotfile_prefix=None
):
...
def getErrorCorrectMapping(cell_barcodes, whitelist, threshold=1):
    ...
    for cell_barcode in cell_barcodes:
        ...
        candidates = [
            white_cell for d, white_cell in tree2.find(cell_barcode, threshold) if d > 0
        ]
...
def getCellWhitelist(
    ...
    if cell_whitelist and error_correct_threshold > 0:
        ...
        true_to_false_map = getErrorCorrectMapping(
            cell_barcode_counts.keys(), cell_whitelist, error_correct_threshold
        )
                ...
                for m in itertools.product(*m_bases):
                    ...
                    if error_barcode in false_to_true_map:
                        ...
                        if false_to_true_map[error_barcode]:
                            U.info(
                                "Error barcode %s can be assigned to more than "
...
def errorDetectAboveThreshold(
    ...
    if resolution_method == "correct":
        ...
        U.info(
            "CBs above the knee discarded due to possible INDELs: %i"
            % error_counter["indel_discarded"]
        )
        U.info(
            "CBs above the knee discarded due to possible errors from "
            "multiple other CBs: %i" % error_counter["error_discarded_mt_1"]
        )
    else:
        U.info(
            "CBs above the knee discarded due to possible errors: %i" % len(discard_cbs)
        )
